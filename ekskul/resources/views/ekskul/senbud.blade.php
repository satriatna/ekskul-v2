...
@section('content')
<title>
    Data Senbud - Aplikasi Pemilihan Senbud UPD
</title>
      ...
      <div class="row">
                    ...
                    <!-- Card body -->
                        <div class="card-body">
                            <div class="row">
                                <div class="col">
                                <h5 class="card-title text-uppercase text-muted mb-0 text-white">Data Senbud </span></h5>
                                </div>
                                <div class="col-auto">
          ...
          <div class="card shadow">
            ...
            <table class="table table-striped table-bordered" id="example"  cellspacing="0" width="100%">
                ...
                <tr class="data-row">
                    ...
                    <td class="align-middle name">{{$s->nama_senbud}}</td>
                    <td class="align-middle hari">{{$s->hari_senbud}}</td>
                    <td class="align-middle kuota">{{$s->kuota_senbud}}</td>
                    <td class="align-middle kuota">{{$s->sisa_kuota_senbud}}</td>
                    <td class="align-middle kuota">
                      @if($s->instruktur_senbud_id == 0)
                        Belum Ada Instruktur
                      @else
                        <a href="/pengguna/detail/{{$s->instruktur_senbud_id}}" class="btn btn-primary btn-sm">Lihat</a>
                      @endif
                    </td>
                    <td class="align-middle">
                    <a href="/senbud/detail/{{$s->id_senbud}}/{{$s->instruktur_senbud_id}}" class="btn btn-info btn-sm">Detail</a>
                    <!-- <a href="/senbud/hapus/{{$s->id_senbud}}" onclick="return confirm('Anda Yakin ?')" class="btn btn-danger btn-sm">Hapus</a> -->
                    </td>
                </tr>
                @endforeach
                </tbody>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Tambah Senbud</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="/senbud/store" method="POST">
        {{csrf_field()}}
        
        <div class="form-group">
                <label for="kuota">Instruktur</label>
                <select name="instruktur_senbud_id" id="instruktur_senbud_id" class="form-control">
        ...
        <div class="form-group">
                <label for="nama"> Nama Senbud</label>
                <input type="text" class="form-control" name="nama_senbud" value="{{ old('nama_senbud') }}">
                @if ($errors->has('nama_senbud'))
                    <div class="error">Gagal : Nama Senbud tersebut telah digunakan sebelumnya</div>
                @endif
            </div>
            <div class="form-group">
